...
```{r}
stand_sf <- sf::st_read("../data/PeSla_Stand_Boundaries.gpkg", quiet = T) %>% 
...
# identical(sf::st_crs(stand_sf), sf::st_crs(treetops_data[[3]]))
# where?
mapview::mapview(
  stand_sf
  , color = "black"
...
plt_stand <- 
  ...
  purrr::map(
    \(x)
    ggplot2::ggplot() +
      ggplot2::geom_sf(data = stand_sf %>% dplyr::filter(poly_name==x), color = "black", fill = NA) +
